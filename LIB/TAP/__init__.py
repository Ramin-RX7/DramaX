"""
COPYRIGHT: https://github.com/swansonk14
GITHUB REPOSITORY: https://github.com/swansonk14/typed-argument-parser
Latest Change: 12/17/2022
"""
"""
Copy Reason:
    1) Unavailable for python 3.11
    2) Minor changes that are listed below
"""
"""
CHANGES:
Line 285:  In the end of "_add_argument()"
"""
    ...
    def _add_argument(self, *name_or_flags, **kwargs) -> None:
        ...
        class newargparser(Tap):
            def __init__(self, *args, **kwargs):
                self.args_list = []
                super(newargparser, self).__init__(*args,**kwargs)

            def _add_argument(self, *name_or_flags, **kwargs) -> None:
            
                rx.style.print(name_or_flags,'red')

                shortened_arg = "-"+name_or_flags[0].replace("-","")[0]

                for arg in name_or_flags:
                    if arg in self.args_list:
                        raise Exception
                        name_or_flags = rx.force(name_or_flags, shortened_arg)
                ...
                super(newargparser, self).add_argument(*name_or_flags, **kwargs)
        """
